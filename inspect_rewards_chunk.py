"""
inspect_rewards_chunk.py
-------------------------
Downloads chunk-UAE6UW5T.js and searches it for:
- guess
- crack
- mastermind
- rewards
- giveaway
Prints all matching strings and surrounding context.
"""

import asyncio
import aiohttp
import re

async def main():
    connector = aiohttp.TCPConnector(ssl=False)
    async with aiohttp.ClientSession(connector=connector) as session:
        url = "https://dash.lucidtrading.com/chunk-UAE6UW5T.js"
        async with session.get(url) as resp:
            content = await resp.text(errors="replace")
            print(f"File size: {len(content)} chars.")
            
            # Print any API paths found
            api_paths = re.findall(r'"/api/[^"]+"|\'/api/[^\']+\'', content)
            print("API Paths found:", api_paths)
            
            # Search for keyword matches
            for word in ["guess", "crack", "mastermind", "rewards", "giveaway", "event", "active"]:
                positions = [m.start() for m in re.finditer(re.escape(word), content, re.IGNORECASE)]
                print(f"\nKeyword '{word}' found at positions: {positions}")
                for p in positions[:8]:
                    snippet = content[max(0, p-120):min(len(content), p+200)]
                    print(f"  [{p}]: {snippet.strip()!r}")

if __name__ == "__main__":
    asyncio.run(main())
