"""
search_all_for_rewards_urls.py
------------------------------
Downloads all unique chunk JS files and searches them for "rewards" or "events"
case-insensitively to find the exact endpoint paths.
"""
...
async def scan_chunk(session: aiohttp.ClientSession, chunk_name: str):
    ...
    try:
        async with session.get(url, timeout=10) as resp:
            ...
            # Find references
            for kw in ["rewards", "events", "redeem-secret", "submit"]:
...
async def main():
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        print("🔍 Searching all chunks for rewards/events/submit/redeem-secret URL strings...")
